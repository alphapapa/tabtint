window.addEventListener("load", function(event) {TabTintOverlay.Initialize();}, false);
window.addEventListener("unload", function(event) {TabTintOverlay.DeInitialize();}, false);

var TabTintWebProgressListener =
    {
        ...
        onStateChange: function(aWebProgress, aRequest, aFlag, aStatus)
        {
            ...
            {
                if (document.defaultView.TabTintOverlay)
                {
                    ...
                    for (iIndex = 0; iIndex < iLen; iIndex ++)
                    {
                        ...
                        {
                            oTab = aTabs[iIndex];
                            setTimeout(function() {document.defaultView.TabTintOverlay.UpdateTabs(oTab);}, 100);
                        }
                    }
                }
            }
...
var TabTintOverlay =
    {
        ...
        Initialize: function()
        {
            ...
            this.oPreferences = this.oPreferences.getBranch("extensions.tabtint.");

            var oContainer = getBrowser().tabContainer;
            if (oContainer) {
                oContainer.setAttribute("tabtinttab", true);
            }
            ...
            this.oPrefObserver =
                {
                    ...
                    {
                        if (topic == "nsPref:changed" && prefName.indexOf("extensions.tabtint.") >= 0)
                        {
                            setTimeout(function() {TabTintOverlay.RefreshAll(false);}, 100);
                        }
                    }
                };
            ...
            this.oStrings = document.getElementById("tabtint-string-bundle");
            ...
            oPrefService.addObserver("extensions.tabtint.fuzzycolormatch", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.x", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.y", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.width", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.height", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.preventwhite", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.preventblack", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.activeonly", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.opacity", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.hue", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.saturation", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.activetabsaturation", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.activetabopacity", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.whitetextthreshold", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.hostnamecache", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.colornavbar", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.activetabbold", this.oPrefObserver, false);
            oPrefService.addObserver("extensions.tabtint.contrastclosebutton", this.oPrefObserver, false);

            var oContainer = getBrowser().tabContainer;
            oContainer.addEventListener("TabOpen", function(event) {event.originalTarget.linkedBrowser.addProgressListener(TabTintWebProgressListener);}, false);
            oContainer.addEventListener("TabClose", function(event) {event.originalTarget.linkedBrowser.removeProgressListener(TabTintWebProgressListener);}, false);
            oContainer.addEventListener("TabSelect", function(event) {TabTintOverlay.UpdateTabs();}, false);

            var aTabs = getBrowser().mTabs;
            var iIndex, iLen = aTabs.length;
            for (iIndex = 0; iIndex < iLen; iIndex ++)
            {
                aTabs[iIndex].linkedBrowser.addProgressListener(TabTintWebProgressListener);
            }
        ...
        InitializeTabContextMenu: function()
        {
            ...
            {
                if (!document.getElementById('tabtint-context-clear'))
                {
                    ...
                    oElement.setAttribute("id", "tabtint-context-clear");
                    oElement.setAttribute("label", this.GetString("tabtint.context.clear"));
                    oElement.addEventListener("command", function() {TabTintOverlay.RefreshAll(true);});

                    oMenuPopup.appendChild(oElement);

                    oElement = document.createElementNS('http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul', "menuitem");
                    oElement.setAttribute("id", "tabtint-context-clearall");
                    oElement.setAttribute("label", this.GetString("tabtint.context.clearall"));
                    oElement.addEventListener("command", function() {TabTintOverlay.RefreshAll(false);});
                    oMenuPopup.appendChild(oElement);
                }
            }
        },
        ...
        {
            var oContainer = getBrowser().tabContainer;
            oContainer.removeEventListener("TabOpen", function(event) {event.originalTarget.linkedBrowser.addProgressListener(TabTintWebProgressListener, Components.interfaces.nsIWebProgress.NOTIFY_STATE_DOCUMENT);}, false);
            oContainer.removeEventListener("TabClose", function(event) {event.originalTarget.linkedBrowser.removeProgressListener(TabTintWebProgressListener, Components.interfaces.nsIWebProgress.NOTIFY_STATE_DOCUMENT);}, false);

            var oPrefService = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranchInternal);
            oPrefService.removeObserver("extensions.tabtint.fuzzycolormatch", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.x", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.y", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.width", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.height", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.preventwhite", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.preventblack", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.activeonly", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.opacity", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.hue", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.saturation", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.activetabsaturation", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.activetabopacity", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.whitetextthreshold", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.hostnamecache", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.colornavbar", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.activetabbold", this.oPrefObserver, false);
            oPrefService.removeObserver("extensions.tabtint.contrastclosebutton", this.oPrefObserver, false);
        ...
        UpdateTabs: function(oTab)
        {
            ...
            var oWindow;
            var oCanvas = document.getElementById("tabtint-canvas");
            ...
            for (iIndex = 0; iIndex < iLen; iIndex ++)
            {
                try
                {
                    if (bHide)
                    {
                        ...
                        {
                            aFrames[iIndex].setAttribute("tabtintdisplay", aFrames[iIndex].style.display);
                    ...
                    else
                    {
                        if (aFrames[iIndex].contentDocument.location.host != oWindow.document.location.host)
                        {
                            aFrames[iIndex].style.display = aFrames[iIndex].getAttribute("tabtintdisplay");
                            aFrames[iIndex].removeAttribute("tabtintdisplay");
                        }
                    }
                } catch (e) {}
            }
...
var EUObjTabTint = {
	...
	try{
	    prefManager.getBoolPref("extensions.tabtint.use");
	}
	catch(ex) {
	    prefManager.setBoolPref("extensions.tabtint.use", true);
	}
    }
}
setTimeout(function() { EUObjTabTint.SetExistingUser() }, 2000);
